...
//        if (isalpha(word_array[i][0])){
        if(current_letter == thread_letter){
...
int main(){
    ...
    pthread_t thread_ids[26];
    // creation of lists of uniq words
        clock_gettime(CLOCK_MONOTONIC, &start_time_par_1);
    for(int i = 0; i < 26; i++){
        int  arg = i;
        pthread_create(&thread_ids[i], NULL, find_unique_words_letters, (void*)arg);
    }
    for(int i = 0; i < 26; i++){
        int  arg = i;
